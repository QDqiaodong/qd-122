...
CREATE TABLE IF NOT EXISTS spring_archive (
    ...
    flag_countersign_operator VARCHAR(32) COMMENT '黄标摘标加签人工号（质量主管）',
    flag_countersign_note VARCHAR(255) COMMENT '黄标摘标加签说明',
    flag_countersign_time DATETIME COMMENT '黄标摘标加签时间（加签水位，晚于该时间闭环的偏离单仍挂黄标）',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
